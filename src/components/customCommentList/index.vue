...
export default {
  ...
  methods: {
    doReplyComment(id) {
      ...
      const parentId = id;
      const level =
        this.currentReplyCommentLevel === 2
          ? this.currentReplyCommentLevel
          : this.currentReplyCommentLevel + 1;
    ...
    showReplyArea(id, level, authorId) {
      // 展示回复框
      const replyAreaShowMap = this.replyAreaShowMap; // 展示当前回复的那一条评论
      const newReplyAreaShowMap = { ...replyAreaShowMap }; // 新的展示当前回复的那一条评论
      for (const key in newReplyAreaShowMap) {
        // 如果当前要回复的评论不是被回复的评论，则将其他评论的回复框隐藏
      ...
      this.$nextTick(() => {
        // 回复的内容框
        this.$refs[ref] && this.$refs[ref][0].focus(); // 回复的内容框获取焦点
      });
      // 回复的内容
      ...
      // 回复的内容
      this.replyContentPlaceholderMap[id] = `回复 @${
        this.currentReplyToAuthorId.nickname
      }（请文明发言）`;
    },
    viewMoreReply(id) {
      // 展示更多的子评论
  ...
  created() {
    ...
    this.commentlist.forEach(item => {
      this.$set(this.replyAreaShowMap, item._id, false); // 展示当前回复的那一条评论
      this.$set(this.replyContentMap, item._id, ""); // 回复的内容
      this.$set(this.viewMoreReplyMap, item._id, false); // 那一条评论的子评论是处于查看更多状态的
      this.$set(this.replyContentPlaceholderMap, item._id, "请文明发言"); // 回复的内容
    });
  }
};
</script>
