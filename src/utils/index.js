...
export function imgRandomName() {
  ...
  const res = `wechat-messger-${timestamp}-${random}`;
  return res;
}
...
const saveFriend = id => {
  ...
  let recentStr =
    storage.getItem("weChatMessager-recentConversation-friend") || "";
  const recentArr = recentStr.split(",");
  if (recentArr.includes(id)) return;
  recentStr = recentStr ? recentStr + "," + id : id;
  storage.setItem("weChatMessager-recentConversation-friend", recentStr);
};

const saveGroup = id => {
  const storage = window.localStorage;
  let recentStr =
    storage.getItem("weChatMessager-recentConversation-group") || "";
  const recentArr = recentStr.split(",");
  if (recentArr.includes(id)) return;
  recentStr = recentStr ? recentStr + "," + id : id;
  storage.setItem("weChatMessager-recentConversation-group", recentStr);
};
