...
export default {
  ...
  methods: {
    ...
    audioCall() {
      // 语音通话
      if (this.isToCoArtBoard || this.isVideoing || this.isAudioing) return; // 如果正在视频通话或者白板协作，则不能语音通话
      this.$store.dispatch("app/SET_IS_AUDIOING", true); // 设置正在语音通话
      this.$eventBus.$emit("web_rtc_msg", { type: WEB_RTC_MSG_TYPE.audio }); // 发送语音通话消息
    },
    toggleShowSettingPanel() {
      // 切换设置面板
      this.showSettingPanel = !this.showSettingPanel; // 切换显示设置面板
    },
    watchDocumentClick() {
