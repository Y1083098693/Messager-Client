<template>
  <div class="recent-conversation-list" v-loading="isLoading">
    <transition-group name="slide-up" appear> // 列表内容
      <conversation-item
    ...
    <div class="messager-svg hor-ver-center" v-if="!outcomeConversationList.length">
      <messager-svg width="200" height="200" />
      <span class="secondary-font">那种城市，有你所想见的那个人儿</span>
    </div>
  </div>
</template>
...
import MessagerSvg from "@/SVGComponents/messager";
export default {
  ...
  methods: {
    async getRecentConversation() {
      ...
      const recentFriendIdsStr =
        window.localStorage.getItem(
          "weChatMessager-recentConversation-friend"
        ) || "";
    ...
    async getRecentGroupConversation() {
      const recentGroupIdsStr =
        window.localStorage.getItem(
          "weChatMessager-recentConversation-group"
        ) || "";
      const recentGroupIds = arrUnique(recentGroupIdsStr.split(",")).filter(
        item => item
      ); // 去重
      this.$http
