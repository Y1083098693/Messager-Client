<template>
  <div class="layout-page" v-css="{'background-image': 'url(' + bgImgUrl + ')'}">
    ...
    <transition name="fade">
      <main v-show="showMain" class="weChat-messager-layout">
        <my-header></my-header>
        ...
        />
        <el-main
          :class="device === 'Mobile' ? 'weChat-messager-main mobile' : 'weChat-messager-main'"
          v-css=" opacity !== 1 ? {'opacity': opacity} : {}"
        >
          <audio :src="NotifyAudio" ref="audio" muted></audio>
          <transition name="slide-left">
            <div
              :class="device === 'Mobile' ? 'weChat-messager-aside mobile' : 'weChat-messager-aside'"
              v-css="device === 'Mobile' ? {'transform': 'translateX(' + asideTranslateX + 'px)'} : ''"
            >
              <my-aside :set-show-theme="setShowTheme" />
            </div>
          </transition>
          <div :class="device === 'Mobile' ? 'weChat-messager-content mobile' : 'weChat-messager-content'">
...
export default {
  ...
  mounted() {
    ...
    console.log(
      `%c Messger V${APP_VERSION} Started %c Contact: cn18077566421@gmail.com %c`,
